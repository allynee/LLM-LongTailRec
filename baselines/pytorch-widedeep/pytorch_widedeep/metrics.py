...
class Coverage(Metric):
    r"""
    Traditional Coverage metric that measures the proportion of unique items that were recommended 
    at least once out of the total number of unique items in the catalog.
    ...
    k: int, Optional, default = 10
        Number of top items to consider for each user.
    ...
    >>>
    >>> coverage = Coverage(n_items_catalog=100, k=5)
    >>> y_pred = torch.rand(100, 50)  # 100 users, 50 item scores
    >>> score = coverage(y_pred)
    >>> print(score)
    """

    def __init__(self, n_items_catalog: int):
    ...
    def __call__(self, y_pred: Tensor) -> np.ndarray:
        """
        ...
        ----------
        y_pred : Tensor
            Tensor containing the predicted scores for all users.
            Shape: (num_users, num_items)

        Returns
        -------
        np.ndarray
            The coverage score as a fraction between 0 and 1.
        """
        # Step 1: Convert probability scores to ranked item indices
        total_num = int(len(y_pred) * 0.2)
        if y_pred.dim() == 1:
            y_pred = y_pred.unsqueeze(0)
        top_k_predictions = torch.argsort(y_pred, descending=True, dim=-1)[:, :len(y_pred)]

        # Step 2: Flatten and add unique items to the set
        recommended_items = top_k_predictions.cpu().numpy().flatten()
        self.recommended_items.update(recommended_items)
